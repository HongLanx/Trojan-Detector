// SSAGenerator: a tool for displaying and interpreting the SSA form of Go programs.
package main
...
Usage: SSAGenerator [-build=[DBCSNFLG]] [-test] [-run] [-interp=[TR]] [-arg=...] package...
Use -help flag to display options.

Examples:
% SSAGenerator -build=F hello.go              # dump SSA form of a single package
% SSAGenerator -build=F -test fmt             # dump SSA form of a package and its tests
% SSAGenerator -run -interp=T hello.go        # interpret a program, with tracing

The -run flag causes SSAGenerator to build the code in a runnable form and run the first
package named main.
...
func doMain() error {
	...
	// Setup output file based on the input filename
	srcFilename := flag.Arg(0) // assumes the first argument is the source file
	outputFilename := strings.TrimSuffix(srcFilename, ".go") + ".ssa"
	...
	// Redirect all fmt outputs to the outputFile
	oldStdout := os.Stdout
