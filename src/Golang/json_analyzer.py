"""
该脚本用于分析和提取Go语言AST（抽象语法树）JSON文件中的关键信息。具体功能包括：
1. 从AST JSON文件中读取数据并提取关键信息，如导入的包（imports）、自定义函数（functions）、
作用域内的对象（scope_objects）、函数调用（calls）和字符串（string）。
2. 提供了递归遍历AST节点的函数，用以提取函数调用和字符串，以及解析复杂的表达式。
3. 每个提取函数都具备递归处理能力，能够从复杂的嵌套结构中提取所需的详细信息。
