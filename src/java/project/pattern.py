...
encryption_patterns = {
    ...
    "FunctionCalls": {
        ...
        "SecureRandom.getInstanceStrong": 6,  # 获取强随机数生成器实例，用于加密操作。
...
botnet_patterns = {
    "Imports": {
        ...
        "java.net.Socket": 6,  # 进行网络通信，可能用于与控制服务器连接。
        "java.net.ServerSocket": 6,  # 监听网络连接，可能用于创建僵尸网络的监听服务。
    },
    "FunctionCalls": {
...
penetration_patterns = {
    "Imports": {
        ...
        'io.netty.channel.*': 7,  # Netty 是一个网络通信框架，用于高效的网络应用开发。恶意使用可能涉及隐藏的网络通信。
        'java.lang.reflect.*': 8,  # 反射 API 用于动态创建实例和调用方法，可能被滥用来隐藏恶意行为。
        "org.apache.http.client.methods.HttpGet": 6,  # 进行 HTTP 请求，可能用于测试漏洞。
        "org.apache.http.client.methods.HttpPost": 6,  # 进行 POST 请求，可能用于渗透测试。
    ...
    "FunctionCalls": {
        ...
        'newInstance': 8,  # 使用反射动态创建实例，可能创建恶意类的实例。
...
obfuscation_patterns = {
    "Imports": {
        "import net.md_5.specialsource.Jar": 4,  # 导入自定义的Jar类，可能涉及JAR文件操作
        ...
        "import org.objectweb.asm.ClassReader": 6,  # ASM 库用于字节码操作，可能表明有修改类文件的意图。
...
phishing_patterns = {
    "Imports": {
       "import android.bluetooth.BluetoothServerSocket": 5,  # 用于创建蓝牙服务器套接字，允许设备等待连接请求，可能用于监听连接，稍有可疑性。
...
"Imports": {
        ...
        "java.util.zip.ZipInputStream": 5,  # ZIP 输入流，用于读取压缩文件。
        "java.util.zip.ZipOutputStream": 5  # ZIP 输出流，用于写入压缩文件。
    },
    "FunctionCalls": {
...
Keyboard_patterns = {
    ...
    "FunctionCalls": {
        ...
        "attach": 7,  # 将文件附加到电子邮件中，用于发送捕获的敏感数据文件。
        "send": 7,  # 发送电子邮件，可能发送包含敏感数据的邮件。
...
rootkit_patterns = {
    ...
    "FunctionCalls": {
        ...
        'call': 8,  # 发送 HTTP 请求并输出响应，可能被用来触发远程命令执行
...
antivirus_patterns = {
    "Imports": {
        'java.io.IOException': 6,  # 用于处理I/O操作的异常情况，可能掩盖潜在的网络通信错误或未授权的操作。
        ...
        'java.io.File': 9,  # 操作文件系统，用于遍历、读取、删除或加密目标文件。
    ...
    "FunctionCalls": {
        ...
        'createKey': 8,  # 生成对称加密的AES密钥，保证文件加密的安全性。
...
adware_patterns = {
    "Imports": {
        ...
        "com.facebook.ads.Ad": 1,  # 用于处理广告对象，可能涉及广告的加载、显示和交互。
        "com.facebook.ads.AdError": 1,  # 用于处理广告加载或显示时的错误，可能用于记录或处理广告错误。
        "com.facebook.ads.AdListener": 1,  # 用于监听广告的事件，如加载、点击和错误，涉及广告生命周期管理。
        "com.facebook.ads.AdView": 1,  # 用于展示Facebook广告的视图，可能用于在应用中展示横幅广告。
        "com.facebook.ads.InterstitialAd": 1,  # 用于处理插页式广告，可能用于在应用中展示全屏广告。
        "com.facebook.ads.InterstitialAdListener": 1,  # 用于监听插页式广告的事件，如加载、显示和关闭，涉及广告生命周期管理。
        "com.facebook.ads.NativeAd": 1  # 用于处理原生广告，可能用于在应用中展示与内容无缝融合的广告。
    },
    "FunctionCalls": {
        'post': 7,  # 涉及到网络通信，尤其是当它与未经授权的数据收集或远程服务器通信相关时
        'parse': 6,  # 连接到Google Play搜索页面并解析应用数据，可能用于抓取和分析应用信息。
        'doInBackground': 6,  # 在后台解析网页，可能隐藏恶意操作，例如数据抓取。
        "onPostExecute": 5,  # 在任务完成后执行，可能用于处理和执行从服务器抓取的命令。
        'bannerAdmob': 4,  # 显示横幅广告，通常无害，但如果频繁调用可能对用户体验产生负面影响。
        'fullAdmob': 5,  # 显示插页式广告，通常无害，但如果频繁调用可能会打断用户体验。
        'nativeAdmob': 4,  # 显示本地广告，通常无害，但如果频繁调用可能对用户体验产生负面影响。
        'MobileAds.initialize': 3,  # 初始化广告SDK，通常无害，但可能用于启动不受欢迎的广告服务。
        "AdView.loadAd": 4,  # 加载广告，通常无害，但如果被滥用可能频繁加载广告以获取广告收入。
        'InterstitialAd.loadAd': 4,  # 加载插页式广告，通常无害，但频繁加载可能打扰用户。
        'InterstitialAd.show': 5,  # 显示插页式广告，可能会打断用户体验，尤其是在不合适的时间点频繁调用时。
        'NativeExpressAdView.loadAd': 4,  # 加载本地广告，通常无害，但如果频繁使用可能影响用户体验。
    ...
    "Strings": {
        ...
        'onAdLoaded': 3,  # 广告加载成功的回调，通常无害，但可能用于记录广告展示次数。
...
hackingtool_patterns = {
    ...
    "FunctionCalls": {
        ...
        'doc.select': 6,  # 选择HTML元素进行操作，可能涉及抓取特定敏感信息。
        'attr("value")': 6,  # 访问HTML元素属性值，可能用于提取敏感数据。
        'Logger.error': 4,  # 记录错误信息，可能用于掩盖恶意操作或记录失败的尝试。
        'Logger.debug': 3,  # 调试信息记录，可能用于恶意软件开发者的调试过程。
